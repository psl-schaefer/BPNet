...
p <- peak_infos %>%
  ...
  theme_bw()
ggsave(filename = paste0(output_dir, "figures/macs2_scores_per_tf.pdf"), 
...
ggsave(filename = paste0(output_dir, "figures/n_peaks_per_tf.pdf"), 
       plot = p, width = 4, height = 4)
p +
  ggdark::dark_theme_linedraw()
```
...
p <- purrr::map_dfr(TFs, function(tf){
  ...
  ggplot() +
  geom_line(aes(x=position, y=value, color=TF), size=0.2) +
...
ggsave(filename = paste0(output_dir, "figures/example_high_q.pdf"), 
...
p <- purrr::map_dfr(TFs, function(tf){
  ...
  theme_bw() +
  theme(plot.title = element_text(size=10, hjust=0.5), strip.background = element_rect(fill=NA))
ggsave(filename = paste0(output_dir, "figures/example_low_q.pdf"), 
       plot = p, width = 4, height = 8)
p + 
  ggdark::dark_theme_linedraw()
```
