...
class ChIP_Nexus_Dataset(Dataset):
    ...
    def __init__(self, set_name: str, input_dir: os.PathLike, TF_list: List[str], subset=True, qval_thr=0.0, subsample=None, seed=42):
        self.indir = input_dir 
        self.set_name = set_name
        self.tf_list = TF_list
        self.one_hot_seqs = np.load(self.indir / f"{self.set_name}_one_hot_seqs.npy") # [batch, bases=4, pwidth=1000]
        self.tf_counts = np.stack([np.load(self.indir / tf / f"{self.set_name}_counts.npy") for tf in self.tf_list], axis=1) # [batch, TF, strand=2, pwidth=1000]
        self.ctrl_counts = np.load(self.indir / "patchcap" / f"{self.set_name}_counts.npy") # [batch, TF, strand=2, pwidth=1000]
        self.ctrl_counts_smooth = _smooth_counts(self.ctrl_counts) # [batch, strand=2, pwidth=1000]
        self.region_info = pd.read_csv(self.indir / "region_info.tsv", sep="\t") # [batch, strand=2, pwidth=1000]
        self.seqnames  = np.genfromtxt(self.indir / f"{self.set_name}_seq_names.txt", dtype=str)
