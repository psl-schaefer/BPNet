...
- Here we generate the model input based on the raw ChIP-Nexus data.
...
knitr::opts_chunk$set(cache = TRUE, 
                      warning = TRUE, 
                      message = TRUE, 
                      cache.lazy = FALSE # by default this is set to true I think
                      )

# tmp for plotting
# load("/Users/pschafer/Learning/BPNet/prc/tmp.RData")
```
...
reticulate::use_condaenv("py-env-bpnet")
...
total_reads <- TFs %>%
  ...
  purrr::map_dbl(function(tf) {
    Rsamtools::countBam(file.path(data_dir, "chip-nexus", tf, "pool_filt.bam"))$records
  })
```

```{r cache=FALSE}
ggplot() +
  geom_bar(aes(y=names(total_reads), x=total_reads/1e6), stat="identity", 
           fill="blue", alpha=0.4, width=0.4) +
  labs(x="Number of Read (x 10^6)", y="TF") +
...
- Check the `macs3 callpeak` documentation [here](https://macs3-project.github.io/MACS/docs/callpeak.html)

```{r}
peak_infos <- purrr::map(TFs, function(tf) {

  narrowPeaks <- 
    rtracklayer::import(file.path(data_dir, "chip-nexus", tf, "idr-optimal-set.narrowPeak.gz"), 
                        format="narrowPeak")
  
  # read in the peak summits and extend symmetrically to get 1000 bp width
  summits <- 
    rtracklayer::import(file.path(data_dir, "chip-nexus", tf, "idr-optimal-set.summit.bed.gz"), 
                           format="bed") %>%
...
write_delim(data.frame(peak_infos) %>% dplyr::mutate(Region = paste0(seqnames, ":", start, "-", end)), 
            file=file.path(output_dir, "region_info.tsv"), delim = "\t")

peak_infos %>% as.data.frame() %>% head
```

## Distribution of the MACS Scores
...
seq_names <- purrr::imap(chrom_list, function(set_chroms, set_name) {
  ...
  write_lines(rnames, file = file.path(output_dir, paste0(set_name, "_seq_names.txt")))
...
one_hot_seqs <- furrr::future_imap(chrom_list, function(set_chroms, set_name) {
  ...
  Biostrings::writeXStringSet(x=peak_seqs,
                              filepath = file.path(output_dir, paste0(set_name, "_seqs.fa")))
...
for key, val in r.one_hot_seqs.items():
  ...
  save_path = Path(r.output_dir) / f"{key}_one_hot_seqs.npy"
  print(f"-> {save_path}")
...
```{r}
peak_infos_reduced <- GenomicRanges::reduce(
  resize(peak_infos, GenomicRanges::width(peak_infos + 1), "start")
  )
peak_infos_reduced
```
...
for tf_name, tf_entry in r.tf_counts.items():
  ...
  for set_name, set_entry in tf_entry.items():
    ...
    save_path = Path(r.output_dir) / tf_name / f"{set_name}_counts.npy"
    print(f"\t-> {save_path}")
...
patchcap_input <- 
  ...
  purrr::set_names() %>%
  purrr::map(function(strand){
  rtracklayer::import.bw(file.path(data_dir, "chip-nexus", "patchcap", paste0("counts.", strand, ".bw"))) %>%
...
for set_name, set_entry in r.ctrl_counts.items():
  mtx = np.stack([set_entry["pos"], set_entry["neg"]], axis=1).astype(np.float32)
  print(f"\t{set_name} shape: {mtx.shape}")
  save_path = Path(r.output_dir) / "patchcap" / f"{set_name}_counts.npy"
  print(f"\t-> {save_path}")
  np.save(file=save_path, arr=mtx)
...
saveRDS(motif_matches, file.path(output_dir, "oct4_sox2_matches.rds"))
motif_matches
```
